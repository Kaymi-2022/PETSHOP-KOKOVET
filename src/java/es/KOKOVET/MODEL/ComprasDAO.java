...
public class ComprasDAO {
    ...
    public List Detalle(int id) {
        List lista = new ArrayList();
        String sql = "select DC.idDetalle, P.idProducto, P.Nombres, DC.idCompras, DC.Cantidad, DC.PrecioCompra FROM detalle_compras DC inner join producto P on P.idProducto = DC.idProducto where idCompras=" + id;
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
    ...
    public int IdPago() {
        ...
        public List Compras() {
        List lista = new ArrayList();
        String sql = "select DC.idCompras, C.Nombres, C.Email, DC.idPago, DC.FechaCompras, DC.Monto,DC.Estado FROM compras DC inner join cliente C on C.idCliente = DC.idCliente";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                Compra1 com = new Compra1();
                com.setId(rs.getInt(1));
                com.setCliente(new Cliente());
                com.getCliente().setNombres(rs.getString(2));
                com.getCliente().setEmail(rs.getString(3));
